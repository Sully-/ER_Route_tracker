// Ajustement manuel pour forcer le point exact
// On va ajuster le coefficient c (offset X) pour que le point soit exactement à 1882
...
// Calculer avec les 4 premiers points
let [a, b, c] = solveLinearSystem(calibrationPoints, p => p.pixelX);
const [d, e, f] = solveLinearSystem(calibrationPoints, p => p.pixelY);

// Ajuster c pour que (11958.53, 10560.359) donne exactement 1882
const testX = 11958.53;
const testZ = 10560.359;
const targetX = 1882;
const currentX = a * testX + b * testZ + c;
const adjustment = targetX - currentX;
c += adjustment;

console.log('Transformation ajustée manuellement:');
...
console.log(`  Ajustement appliqué: ${adjustment.toFixed(2)}px`);
console.log();
...
console.log('Erreurs sur les points de calibration originaux:');
